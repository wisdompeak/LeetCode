...
本题要求在nums数组里找一个最长的子序列，使得其XOR Sum等于target。
...
考虑到nums的数值不超过1e4，那么所有XOR Sum的种类也不会超过1e4. 我们可以用常规的DP。令dp[i][v]表示对于前缀nums[0:i]，Xor Sum等于v时的最长子序列长度。状态转移的依据就是当前元素的“选或不选”两种决策：
```
